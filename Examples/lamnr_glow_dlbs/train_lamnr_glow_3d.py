...
def main():
    ap = argparse.ArgumentParser("Glow 3D (builder) trainer")
    
    # --- Input Data & Geometry ---
    ap.add_argument("--view", action="append", nargs="+", required=True,
                    help="Path patterns for each imaging modality (e.g., T1, FA). Use one --view per modality. Files are matched across views by subject directory structure.")
    ap.add_argument("--H", type=int, default=128, 
                    help="Target image height (pixels) for resizing inputs.")
    ap.add_argument("--W", type=int, default=128, 
                    help="Target image width (pixels) for resizing inputs.")
    ap.add_argument("--D", type=int, default=128, 
                    help="Target image depth (slices) for 3D volumes. Ignored if --spatial-dims is 2.")
    ap.add_argument("--spatial-dims", type=int, choices=[2, 3], default=2,
                    help="Dimensionality of the data. Use 2 for independent slice training or 3 for full volumetric training.")
    ...
    ap.add_argument("--glowbase-min-log", type=float, default=-5.0, 
                    help="Minimum clamp value for log-scale parameters to prevent numerical underflow.")
    ap.add_argument("--glowbase-max-log", type=float, default=5.0, 
                    help="Maximum clamp value for log-scale parameters to prevent numerical explosion.")
    ...
    ap.add_argument("--net-actnorm", action="store_true", 
                    help="If set, inserts ActNorm layers inside the coupling subnetworks (not just the main flow).")

    # --- Training Loop & Logistics ---
    ap.add_argument("--batch", type=int, default=32, 
                    help="Per-GPU batch size. Reduce this if encountering CUDA Out Of Memory (OOM) errors.")
    ...
    ap.add_argument("--eval-interval", type=int, default=1000, 
                    help="Frequency (in iterations) to run validation, calculate NLL, and update the learning rate scheduler.")
    ...
    ap.add_argument("--precision", type=str, default="mixed", choices=["double","float","mixed"], 
                    help="Floating point precision. 'mixed' uses AMP (Automatic Mixed Precision) for speed.")
    ap.add_argument("--amp-dtype", type=str, default="bf16", choices=["bf16","fp16"], 
                    help="Data type for AMP. bf16 is highly recommended for stability on Ampere+ GPUs; fp16 otherwise.")
    ...
    ap.add_argument("--plateau-factor", type=float, default=0.5, 
                    help="Factor by which the learning rate is reduced when validation NLL plateaus (ReduceLROnPlateau).")
    ...
    # --- Gradients & Exponential Moving Average (EMA) ---
    ap.add_argument("--grad-clip", type=float, default=2.0, 
                    help="Maximum L2 norm for gradient clipping. Crucial for preventing exploding gradients in Normalizing Flows.")
    ap.add_argument("--grad-accum", type=int, default=1,
                    help="Accumulate gradients over N steps before updating weights. Simulates a larger batch size (Effective Batch = batch * grad-accum).")
    ap.add_argument("--ema", action="store_true", 
                    help="Maintain an Exponential Moving Average of model weights. Recommended for smoother sampling.")
    ap.add_argument("--ema-decay", type=float, default=0.9995, 
                    help="Decay rate for the EMA weights. Higher values (e.g., 0.9999) mean slower updates but more stability.")

    # --- Checkpointing & I/O ---
    ap.add_argument("--resume", type=str, default="", 
                    help="Explicit path to a .pt checkpoint file to resume training.")
    ap.add_argument("--auto-resume", action="store_true", 
                    help="If set, automatically looks for 'training_state.pt' in the --out-dir to resume interrupted runs.")
    ...
    ap.add_argument("--slice-idx", type=int, default=120, 
                    help="Z-axis index to extract from 3D NIfTI files when training in 2D mode (--spatial-dims 2).")
    ap.add_argument("--val-frac", type=float, default=0.10, 
                    help="Fraction of the subject cohort to randomly hold out for validation (e.g., 0.10 = 10%).")
    ap.add_argument("--subject-limit", type=int, default=0, 
                    help="Limit the dataset to N subjects for quick debugging. 0 means use all available subjects.")
    ...
    ap.add_argument("--align-warmup", type=int, default=500, 
                    help="Number of iterations where alignment loss is scaled from 0 to full strength to prevent early collapse.")
    ap.add_argument("--proj-dim", type=int, default=256, 
                    help="Output dimensionality of the projection MLP used before calculating alignment losses.")
    ap.add_argument("--proj-hidden", type=int, default=512, 
                    help="Hidden layer size of the projection MLP.")
    ap.add_argument("--temperature", type=float, default=0.1, 
                    help="Temperature parameter for InfoNCE contrastive alignment loss.")
    ap.add_argument("--barlow-lambda", type=float, default=5e-3, 
                    help="Weighting factor for the off-diagonal (redundancy reduction) terms in Barlow Twins loss.")
    ap.add_argument("--weighting", choices=["fixed","kendall"], default="fixed", 
                    help="Strategy for balancing NLL and Alignment losses. 'kendall' learns the weighting dynamically via homoscedastic uncertainty.")
    ap.add_argument("--init-logvar-nll", type=float, default=0.0, 
                    help="Initial log variance parameter for NLL when using Kendall weighting.")
    ...
    ap.add_argument("--use-ckpt-config", action="store_true",
                help="When resuming, override current architectural args (L, K, hidden, etc.) with those saved in the checkpoint to prevent shape mismatches.")

    # --- ANTs Data Augmentation ---
    ap.add_argument("--aug-schedules", type=str,
        ...
        help="Schedules to gradually reduce data augmentation intensity during training (Format: param:curve:start->end@iters).")
    ap.add_argument("--disable-aug-anneal", action="store_true", 
                    help="If set, ignores --aug-schedules and uses static, unvarying augmentation intensities throughout training.")

    # --- Previews & Sampling ---
    ap.add_argument("--sample-mode", type=str, choices=["model","data","off"], default="model",
                    help="Determines what is plotted during eval. 'model' generates new samples from the prior, 'data' plots validation reconstructions.")
    ap.add_argument("--sample-temp", type=float, default=1.0,
                help="Temperature multiplier for latent noise during 'model' sampling. <1.0 yields sharper, less diverse images.")

    # --- Subspace Screening ---
    ap.add_argument("--screen", type=str, default="none", choices=["none","cca","hsic"],
                    help="Method to find shared information between views *before* applying alignment loss. Focuses alignment on relevant features.")
    ap.add_argument("--screen-warmup", type=int, default=1000,
                    help="Number of iterations to wait before performing the first subspace screening analysis.")
    ap.add_argument("--screen-refresh", type=int, default=0,
                    help="Frequency (in iterations) to re-evaluate the shared subspace. 0 means it is only calculated once after warmup.")
    ap.add_argument("--screen-frac", type=float, default=0.5,
                    help="Fraction of the total projected dimensions to identify and retain as 'shared' information between views.")
    ap.add_argument("--cca-ridge", type=float, default=1e-3,
                    help="Ridge regularization penalty for Canonical Correlation Analysis (CCA) to ensure matrix invertibility.")
    ap.add_argument("--prefilter-frac", type=float, default=0.5,
                help="Fraction of dimensions to keep when using the Pearson pre-filter before HSIC calculation.")    
