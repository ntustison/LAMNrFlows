...
Let $(V)$ modalities $({x^{(v)}}_{v=1}^V)$. Flow $(f)$ factorizes into levels $(f_\ell)$, 
yielding per-level latents \(Z_\ell^{(v)}=f_\ell(x^{(v)})\). Lightweight projector \(P_\ell\) (shared or per-view) produces \(\tilde Z_\ell^{(v)}=P_\ell Z_\ell^{(v)}\).  
...
- **Barlow Twins (multi).** Push cross-correlation toward identity; penalize off-diagonals; decorrelate.  
- **VICReg (multi).** Invariance + variance floor + covariance shrinkage to prevent collapse.  
- **InfoNCE (multi).** Contrastive with in-batch negatives; temperature control; batch-size sensitive.  
- **HSIC (biased).** Kernel dependence measure capturing higher-order relations.  
**Implementation notes.** Feature normalization; projector depth 1–2; temperature schedules; covariance shrinkage for Barlow/VICReg.
...
## Figure placeholder
- **Fig. 1** CGM Pipeline schematic (encode → CCA subspace → dataset stats → conditional Gaussian → samples/mean → decode).  
  - See companion image: `cgm_pipeline_schematic.pdf` / `cgm_pipeline_schematic.png`.

---

## Notes for writing
- Use neutral, methods-first tone.  
- Keep Kendall–Gal framing concise; emphasize *aleatoric* weighting for alignment, likelihood unweighted.  
- Defer imputation results to a later section once finalized.
