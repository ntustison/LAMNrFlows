...
def _read_image_any(path: Path, slice_axis: int, slice_index: int, target_hw: Optional[Tuple[int, int]] = None, mask_background: bool=False) -> torch.Tensor:
    ...
    if is_nii:
        ...
        if target_hw is not None:
            H, W = target_hw
            resize_factor = min(float(H)/float(img2d.shape[0]), 
                                float(W)/float(img2d.shape[1]))
            spacing = (img2d.spacing[0] / resize_factor, 
...
def main_gauss_impute(argv=None):
    ...
    ap.add_argument("--target", type=str, required=True,
                    help="Comma-separated list of views to be imputed (e.g., 'T2,FA').")
    ap.add_argument("--slice-axis", type=int, required=True,
